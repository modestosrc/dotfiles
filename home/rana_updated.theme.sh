...
function _omb_theme_PROMPT_COMMAND {
  ...
  PS1+=$'\n'"${D_INTERMEDIATE_COLOR}󱆃  ${D_DEFAULT_COLOR}"

  PS2="${D_INTERMEDIATE_COLOR}󱆃  ${D_DEFAULT_COLOR}"
}
...
# Function to set the terminal prompt according to vi mode
function set_prompt {
    local mode
    if [[ $1 == 'i' ]]; then
        mode="[I]"
...
function insert_mode_prompt_info {
    set_prompt 'i'
}

bind -m vi-insert '""\e": "insert_mode_prompt_info"'
bind -m vi-command '""\e": "vi_mode_prompt_info"'
