...
autorunApps = {
    ...
    "xmodmap /home/mateus/.Xmodmap",
}

if autorun then
    for app = 1, #autorunApps do
        awful.util.spawn(autorunApps[app])
        naughty.notify({ text = "Running: " .. autorunApps[app] })
    end
end
...
awful.screen.connect_for_each_screen(function(s)
                    ...
                    {
                        {
                            id            = 'icon_role',
                            widget        = wibox.widget.imagebox,
                            forced_width  = 30, -- largura desejada
                            forced_height = 30, -- altura desejada
                        },
                        margins = 2,
    ...
    s.mywibox:setup {
        ...
        { -- Right widgets
            ...
            layout = wibox.layout.fixed.horizontal,
            awful.widget.watch("bash -c \"pactl get-sink-volume @DEFAULT_SINK@ | awk '{print $5}'\"", 0.2, function(
                widget,
                stdout)
                widget:set_text(" Volume: " .. stdout)
            end),
            separator,
            awful.widget.watch("bash -c \"sensors | awk '/id 0/ {print $4}'\"", 10, function(
                widget, stdout)
                widget:set_text(" Temp CPU: " .. stdout .. "°C ")
            end),
            separator,
            awful.widget.watch("bash -c \"free -h | awk ' /Mem/ {print $3}\'\"", 5, function(widget, stdout)
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey }, "r", function()
            awful.spawn("/home/mateus/.config/rofi/launchers/type-4/launcher.sh")
        end,
        { description = "show the menubar", group = "launcher" })
